          ...
          <React.Fragment key={row[1]}>
            {index === 4 ? (
              <View key={row[1]} style={[styles.buttonsContainer, { width: '74%' }]}>
                {row.map(item => {
                  return (
                    <Button shape={buttonShape.SQUARE} onPress={() => onPressHandler(item)} title={item} key={item} />
                  );
                })}
              </View>
            ) : (
              <View key={row[1]} style={styles.buttonsContainer}>
                {row.map(item => {
                  if (index === 0) {
                    return (
                      <Button shape={buttonShape.ROUND} onPress={() => onPressHandler(item)} title={item} key={item} />
                    );
                  }
                  if (item === '+' || item === '=') {
                    return (
